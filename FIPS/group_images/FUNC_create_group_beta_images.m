...
for cond_idx = 1 : length(cond_list)
    ...
    % 2019-08-20: adding saving of subject T-files as well for later use
    T_name =            sprintf('spmT_%4.4i.nii', cond_list(cond_idx) );
    ...
    for subject_idx = 1 : length(subject_list)
        ...
        % 2019-08-20
        T_path =            fullfile(target_path, T_name);
        new_filepath =      fullfile(indiv_subj_store_path, [subject_list{subject_idx} '_' T_name]);
