...
% Doesn't currently do anything as MRI_Info.epi_run_list is always a cell array
...
for irun = 1:length(run_name_list)
    ...
    if isempty( [strfind(run_name_list{irun}, '\') strfind(run_name_list{irun}, '/')] );
    
%     if ~isdir(run_name_list{irun})
        run_name_list{irun} = [MRI_Info.epi_path run_name_list{irun}];%= [MRI_Info.epi_path filesep run_name_list{irun}];
    end
    
%     all_niis = search_dir(run_name_list{irun},'*MB.nii'); % is recursive
...
% List of open inputs
if ~isempty(strfind(MRI_Info.spm_job, 'Batches') )
    jobfile = {MRI_Info.spm_job};
else
...
for ifile = 1:length(MRI_Info.epi_full_file_names)
    ...
    % 2016-03-08 Royston
    MRI_Info_current = MRI_Info;
    onsets_holder = MRI_Info.ExpDef_event_onsets;
    cell_check = iscell(onsets_holder);
    if cell_check == 1
        MRI_Info_current.ExpDef_event_onsets = cell2mat( MRI_Info.ExpDef_event_onsets(ifile) );
    else
        MRI_Info_current.ExpDef_event_onsets = MRI_Info.ExpDef_event_onsets;
    end
    
%     MRI_Info_current.T_names = {'Simple', 'Goal', 'Audio', 'Stim'};
    ...
    if exist_path == 7
        try
            ...
            % 2017-11-17 Royston
            cd(file_path);
            save('Master_Processing_Parameter.mat','MRI_Info_current','current_epi'); % the spm job which will read in Master_Processing_Parameter.mat
            spm('defaults', 'FMRI');
            spm_jobman('serial',jobs, '', inputs{:});%THIS IS THE UNDEBUGGABLE BASTARD THAT RUINS EVERYTHING
